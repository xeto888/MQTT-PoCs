...
sleep 5
mosquitto_sub -h 127.0.0.1 -p 666 -t regist -u xeto888 -P <MYPW> | while read -r line
...
do
  ...
  if grep -q "BEGIN CERTIFICATE" decoded_payload; then
    ...
    mosquitto_pub -h 127.0.0.1 -p 666 -t Information -u xeto888 -P <MYPW> -m "Registration Completed"
    mosquitto_pub -h 127.0.0.1 -p 666 -t Information -u xeto888 -P <MYPW> -m $b64_cert
    rm decoded_payload
    rm received_pubkey
    rm encrypted_certs

  else 
    mosquitto_pub -h 127.0.0.1 -p 666 -t Information -u xeto888 -P <MYPW> -m "Invalid Public Key"
  fi
  echo "" > cmds
done
